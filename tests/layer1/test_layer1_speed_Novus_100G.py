...
def get_port_locations(location):
    ...
    for rg in range(1, 9):
        rg_fanout4[rg] = list(range(rg_fanout4_starting_port,
                                    rg_fanout4_starting_port + 4))
        rg_fanout4_starting_port += 4

    for rg in range(1, 9):
        rg_fanout2[rg] = list(range(rg_fanout2_starting_port,
    ...
    if port_num < 9:
        ...
        port_40g_100g = next(key for key in rg_fanout4
                             if port_num in rg_fanout4[key])
        port_50g = rg_fanout2[port_40g_100g][0]
    elif port_num > 40 and port_num < 57:
        port_40g_100g = next(key for key in rg_fanout2
                             if port_num in rg_fanout2[key])
        port_10g_25g = rg_fanout4[port_40g_100g][0]
        port_50g = port_num

    port_40g_100g = ";".join(location.split(";")[0:2]) + \
        ";" + str(port_40g_100g)
