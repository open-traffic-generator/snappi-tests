...
def test_pfc_unpause_e2e(api, settings, utils, lossless_priorities):
    ...
    utils.wait_for(
        lambda: results_ok(api, utils, packets, lossless_priorities),
...
def results_ok(api, utils, packets, lossless_priorities, check_for_pause=True):
    ...
    pause = [f.frames_rx for f in flow_results if f.name == "rx_pfc_pause"][0]
    un_pause = [
